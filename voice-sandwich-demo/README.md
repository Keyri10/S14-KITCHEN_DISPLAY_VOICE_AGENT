...
Open a second browser tab at **http://localhost:8000/kitchen** while the server is running.

1. In the main app, start a voice session and place an order (ask the assistant to add items, then confirm).
2. When `confirm_order` runs, the order appears on `/kitchen` automatically via WebSocket (`/ws/kitchen`).
3. Kitchen staff can move orders: **Nuevo** → **En preparación** → **Listo** (updates broadcast to all connected kitchen screens).

| Endpoint | Purpose |
|----------|---------|
| `GET /kitchen` | Kitchen UI (SPA) |
| `WebSocket /ws/kitchen` | Real-time `order_new`, `order_updated`, initial `snapshot` |
| `GET /api/kitchen/orders` | List orders (query `?active=false` includes completed) |
| `PATCH /api/kitchen/orders/:id` | Update status (`nuevo`, `en_preparacion`, `listo`) |

**Demo flow (3 cases):**

- **Caso 1:** One confirmed order → appears instantly on `/kitchen`.
- **Caso 2:** Multiple voice sessions / confirmations → all orders listed in columns by status.
- **Caso 3:** Click status buttons on a card → all kitchen tabs update without refresh.
...
│       ├── kitchen/     # Order store + kitchen WebSocket hub
